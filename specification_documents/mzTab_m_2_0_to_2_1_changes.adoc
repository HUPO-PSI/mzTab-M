...
| Metadata (MTD) Section | 62 | 76 | 16 | 2 | 60
...
| `study_variable_group[1-n]-study_variable_refs`
a| Bar-separated references to the study variables that belong to this study variable group. This is the sole link between study variables and their group: each study variable MUST be referenced by exactly one study variable group.
| Integer List
| False
| —
...
a| Additional parameters or factors, separated by bars, that are known about study variables allowing the capture of more complex, such as nested designs.
| Param List
| False
| —
...
==== `study_variable[1-n]-factors` 🔴 _(Removed in v2.1)_
...
==== `ms_run[1-n]-parameters` 🟢 _(Added in v2.1)_
...
==== `assay[1-n]-protocol_refs` 🟢 _(Added in v2.1)_
...
==== `assay[1-n]-parameters` 🟢 _(Added in v2.1)_
...
==== `study_variable[1-n]-ms_run_refs` 🟢 _(Added in v2.1)_
...
==== `study_variable_group[1-n]` 🟢 _(Added in v2.1)_
...
MTD	study_variable_group[1]-study_variable_refs	study_variable[1]
...
MTD	study_variable_group[2]-study_variable_refs	study_variable[2]
...
==== `study_variable_group[1-n]-description` 🟢 _(Added in v2.1)_
...
==== `study_variable_group[1-n]-type` 🟢 _(Added in v2.1)_
...
==== `study_variable_group[1-n]-datatype` 🟢 _(Added in v2.1)_
...
COM	each group lists its member study variables (top-down link):
MTD	study_variable_group[1]-study_variable_refs	study_variable[1]
MTD	study_variable_group[4]-study_variable_refs	study_variable[2]\{vbar} study_variable[3]
...
==== `study_variable_group[1-n]-unit` 🟢 _(Added in v2.1)_
...
==== `study_variable_group[1-n]-study_variable_refs` 🟢 _(Added in v2.1)_
...
a| ....
----
MTD	study_variable_group[1]-study_variable_refs	study_variable[1]\{vbar} study_variable[2]
----
....
...
==== `protocol[1-n]-parameters` 🟢 _(Added in v2.1)_
...
SML	...	CID:00027395\{vbar}HMDB:HMDB0001847\{vbar}null	...
----
....
...
NOTE: ⚠️ Changed fields: Mandatory, Nullable, Example
...
a| *TA measure of the variability of the study variable abundance measurement, calculated using the method as described in the metadata section (study_variable[1-n]_average_function), with a default = arithmethic co-efficient of variation of the small molecule’s abundance in the given study variable.*
