...
| Metadata (MTD) Section | 67 | 76 | 11 | 2 | 65
...
| `mzTab-profile`
a| The profile of the mzTab-M file, declaring which tables are present and which validation rule set applies. +
MUST be one of: M (MTD only), M+S (MTD + SML), M+F (MTD + SMF), M+F+E (MTD + SMF + SME) or M+S+F+E (all four tables; the richest profile, corresponding to the structure recommended in mzTab-M 2.0/2.1). +
OPTIONAL; if absent (e.g. legacy mzTab-M 2.0 files, or files written before profiles were introduced), validators MUST infer the profile from the tables present and report it in the validation log. New files SHOULD set this field explicitly.
| String
| False
| —
...
a| *Mandatory when the file contains quantified molecules (an SML or SMF section is present); optional otherwise. The quantification method used in the experiment reported in the file.*
...
a| *Mandatory when the Small Molecule (SML) section is present; optional otherwise. Defines what type of units are reported in the small molecule summary quantification / abundance fields*
...
a| *Mandatory when the Small Molecule Feature (SMF) section is present; optional otherwise. Defines what type of units are reported in the small molecule feature quantification / abundance fields.*
...
a| *Mandatory when the Small Molecule Evidence (SME) section is present; optional otherwise. Small molecule identification confidence metrics.<br/>Scoring System +
...
==== `mzTab-profile` 🟢 _(Added in v2.1)_
...
a| ....
----
MTD	mzTab-profile	M+F
MTD	mzTab-profile	M+S+F+E
----
....
