...
mzTab-M 2.1.0 clarifies several aspects, and introduces the `spectral_variable_group` to support multi-factorial experimental designs. 
...
* **categorical** – `[STATO, STATO_0000252, categorical variable]` is a variable that can only assume a finite number values without inherent ordering. Synonym "nominal" variable. In programming languages often represented as enums, factors (in R), or strings.

* **ordinal** – `[STATO, STATO_0000228, ordinal variable]` Ordered categorical variable where the sequence matters but intervals between levels are undefined. These values represent categories with a defined sequence or ranking. Example: disease severity (mild, moderate, severe), confidence levels (low, medium, high).

* **continuous** – `[STATO, STATO_0000251, continuous variable]` is a variable that can take any value within a range on a continuous scale. Example: temperature, height, concentration, time.

//#* **logical** – Boolean/logical values (true/false, yes/no).
...
|*Example* a|
....
MTD study_variable_group[1]-type [STATO, STATO_0000252, categorical variable]
MTD study_variable_group[2]-type [STATO, STATO_0000228, ordinal variable]
MTD study_variable_group[3]-type [STATO, STATO_0000251, continuous variable]
....
...
[cols="20,80",]
|=============================================================================
|*Description* |The datatype of the group variable, which determines how they can be encoded and parsed in mzTab-M files, and how the values are handled in programming languages. Datatypes are specified as W3C XML Schema datatypes.
...
* xsd:dateTime – Combined date and time, optional timezone.
...
[cols=","]
|=======================================================================================================================================================================
|*Description* |Additional parameters or factors, separated by bars, that are known about study variables allowing the capture of more complex, such as nested designs. *This field is deprecated and will be removed in a future release*.
|*Type* |Param List
|*Mandatory* |False
|*Example* a|
[subs="verbatim,quotes"]
