...
[cols=",",]
|=============================================================================
|*Description* |A reference to the study_variable_group that this study variable belongs to. This field is MANDATORY if study_variable_group(s) are defined, allowing study variables to be linked to the experimental design factors they represent.
...
[cols=",",]
|=============================================================================
|*Description* |A parameter defining the group to which the study variable belongs. This allows grouping of related study variables that belong to the same experimental design factor in multi-factorial designs. The parameter can be either a CV Parameter or a user-defined parameter. Added in mzTab-M 2.1.
|*Type* |Parameter
|*Mandatory* |False
|*Example* a|
....
...
[cols=",",]
|=============================================================================
|*Description* |The data type of the group variable, which determines how the values should be interpreted or encoded/decoded. This field is MANDATORY when study_variable_group is defined to ensure unambiguous interpretation. The following data types are supported:

* **nominal** – Categorical values without inherent ordering. No specific ordering is assumed and values represent categories without a defined sequence. Example: sex (male, female), tissue type (liver, kidney).

* **ordinal** – Ordered categorical values where the sequence matters but intervals between levels are undefined. These values represent categories with a defined sequence or ranking. Example: disease severity (mild, moderate, severe), confidence levels (low, medium, high).

* **categorical** – Categorical values with alphabetically ordered levels. This ensures consistent encoding across different implementations. The base level is defined alphabetically (i.e., the first value alphabetically becomes the reference level). Example: genotype (WT, mutant), treatment condition.
...
MTD study_variable_group[2]-type ordinal
MTD study_variable_group[3]-type categorical
....
|=============================================================================
