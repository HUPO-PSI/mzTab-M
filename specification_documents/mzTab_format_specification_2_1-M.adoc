...
Since an MS run may be represented in several formats (with potentially slightly different data associated with the spectrum), a format suffix MAY be specified (e.g. `mzspec:<collection>:<msRun>.RAW`) to signify that a specific file type is meant. More info on the standard can be found: https://www.psidev.info/usi. 
Laslty, and if specified, the format suffix should align with the ms_run[1-n]-format parameter and the ms_run[1-n]-location suffix defined in the metadata section.
|*Type* |String
|*Mandatory* |False
|*Example* a|
....
